...
internal class ModuleDescriptorReader : IModuleDescriptorReader {
    ...
    companion object {
        val MODULE_DESCRIPTOR_TYPE_READER: ObjectReader = kotlinObjectMapper.reader().forType(LoadableModuleDescriptor::class.java)
    }
    ...
    override fun readFromModuleJar(moduleClassLoader: URLClassLoader): Result<LoadableModuleDescriptor> {
        val moduleDescriptorResource = moduleClassLoader.getResource(MODULE_DESCRIPTOR_PATH)
            ?: return Result.failure(
                BasicModuleException("Can't locate $MODULE_DESCRIPTOR_PATH into '${moduleClassLoader.urLs[0]}'. May be jar is not loadable module?")
            )
        return try {
